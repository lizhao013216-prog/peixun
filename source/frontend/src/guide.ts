...
export const tours = [
  {
    ...
    color: "blue",
    duration: "约 5–8 分钟讲解",
    summary: "从领域素材与独立仿真工程，到课程发布和学员操作。",
    ...
    steps: [
      step(
        ...
        [
          "查看本系统自有与已授权素材。",
          "需要演示入库时，点击上传或导入按钮创建私有素材。",
        ],
        "资源有领域归属、版本和共享范围，已批准后可被工程引用。",
        "说明素材默认私有，跨系统复用必须显式共享且不能修改源资源。",
      ),
      step(
        "创建独立仿真工程",
        "S102",
        ...
        "工程拥有独立场景、对象和联动规则，刷新后仍保持，调试实例互不影响。",
        "强调保存或调试一个工程不会改变同系统或其他系统的工程。",
      ),
      step(
        "检查并发布不可变模板",
        "S103",
        "AUTHOR",
        [
          "在工程点击“发布为模板”，填写使用说明并执行“检查配置”。",
          "确认发布后进入模板库，查看对象、规则、素材依赖、来源工程和发布信息。",
          "从模板新建工程或创建修订，确认历史版本内容不变。",
        ],
        "模板形成独立的不可变版本，可按版本调试和复制复用。",
        "说明跨系统复用同时校验模板授权和全部素材依赖；P3课件会明确绑定这里的发布版本。",
      ),
      step(
        "编排操作课程",
        "S104",
        "AUTHOR",
        [
          "点击“新建操作课件”，课件契约选择“V2 · 原训练兼容课件”。",
          "查看 8 个步骤，点击“提交审核”。",
        ],
        "课程进入“待审核”，制作和审核由不同身份完成。",
        "这条金标流程继续演示V2兼容课件；V3模板绑定课件也已支持按发布快照分配、训练、计分和归档。",
      ),
      step(
        "教员审核",
        "C04",
      ...
      step(
        ...
        "LEARNER_A",
        [
          "在操作课程任务行点击“查看任务并开始”；已完成任务可点“新建补训”。",
          "选择“提示操作”和“个人”计分范围，点击弹窗“开始训练”。",
        ],
        "进入该课程的训练页面，产生一次独立训练记录。",
        "点击操作课程任务的“查看训练结果”，查看已有训练结果。",
      ),
      step(
        "完成 8 步操作",
        "C05",
        "LEARNER_A",
        [
          "每步按“本步目标”选择对象，点击具体动作按钮；阅读成功结果后点“继续下一步”，共8步。",
          "完成后点击“导出训练报告”。",
        ],
  ...
  },
  {
    ...
    steps: [
      ...
      step(
        ...
        "AUTHOR",
        [
          "点击“新建维修课件”，课件契约选择“V2 · 原训练兼容课件”，并选择上一步的培训需求。",
      ...
      step(
        ...
        "S202",
        "INSTRUCTOR",
        ["返回维修课件，选择已发布课程，点击“分配培训任务”，选择学员A并确认。"],
        "生成维修培训任务。",
      ...
      step(
        ...
        "LEARNER_A",
        [
          "找到维修任务，点击“查看任务并开始”或“新建补训”。",
          "选择“提示操作”和“个人”计分范围，开始训练。",
        ],
        "创建独立的个人维修训练记录。",
        "点击维修任务“查看训练结果”，查看已有的 88 分案例。",
      ),
      step(
        "演示错误与帮助计分",
        "C05",
        "LEARNER_A",
        [
          "第1步先选阀门，提交两次错误操作，再点击“查看操作提示（扣2分）”一次。",
          "改选通用泵组，完成第1步。此后按每步目标选择对象、执行动作、查看结果、继续下一步；完成后填写课程反馈。",
        ],
...
export function guidePath(item = currentStep()) {
  ...
  const domain: SystemId =
    guide.tour === "operation" ? "OPERATION" : "MAINTENANCE";
  ...
  if (["S104", "S202"].includes(item.page)) {
    id = course?.id;
    store.selectedCourseBySystem[domain] = id || "";
  }
  if (item.page === "C05") {
    id = s.attempts
      .filter((a: any) => a.courseId === course?.id && a.status === "COMPLETED")
      .at(-1)?.id;
    store.selectedAttemptBySystem[domain] = id || "";
  }
