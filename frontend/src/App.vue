  ...
  <div class="app-shell">
    <aside class="sidebar" :class="{ open: sidebarOpen }">
      <a class="brand" href="/workbench" @click.prevent="go('C01')"
        ...
        <div><strong>航维</strong><span>保障业务共性平台</span></div>
        <small>DEMO</small></a
      >
      ...
      <main>
        <div class="page-heading">
          ...
          <div class="heading-actions">
            ...
            ><button
              class="btn"
              :class="store.guide ? 'primary' : 'secondary'"
              @click="store.guide = !store.guide"
            >
              <Icon name="Presentation" :size="16" />演示指引
            </button>
          </div>
        </div>
        <div v-if="store.guide" class="guide-strip">
          <div>
            <b>完整业务演示</b><span>沿着业务链逐步完成，每个环节独立保存</span>
          </div>
          <button
            v-for="(g, i) in [
              ['S104', '操作课程'],
          ...
          >
            <i>{{ i + 1 }}</i
            >{{ g[1] }}<Icon name="ChevronRight" :size="13" />
          </button>
        </div>
    ...
    <Modal
      ...
      ><div class="prose">
        ...
        <h4>如何开始</h4>
        <p>
          点击“演示指引”，依次完成课程、任务、准备、培训、演练和施工归档。可通过右上角切换内置身份，体验制作、审核、训练及验收职责。
        </p>
        <h4>数据与平台能力</h4>
        <p>
