...
const course = computed(
  () =>
    courses.value.find(
      (c: any) => c.id === (store.selectedCourse || route.params.id),
    ) || courses.value.at(-1),
);
const role = computed(
...
function selectCourse(e: Event) {
  ...
  store.selectedCourse = (e.target as HTMLSelectElement).value;
}
...
async function revise() {
  ...
  if (r) store.selectedCourse = r.id;
}
</script>
<template>
  <section class="business-intro">
    <div>
      <span class="task-eyebrow"
        >教员工作区 ·
        {{ domain === "OPERATION" ? "操作课程" : "维修课件" }}</span
      >
  ...
  <section v-if="!course" class="panel empty-state">
  ...
  <template v-else>
    ...
    <section v-if="assigned.length" class="info-note course-handoff">
      ...
      <button class="btn primary" @click="router.push(pathFor('S203'))">
        查看学员任务</button
      ><button class="btn secondary" @click="router.push(pathFor('S204'))">
        查看培训结果
      </button>
    </section>
    <section class="panel">
